...
export function Header() {
  ...
  const pill = (path: string) =>
    `${isActive(path) ? 'bg-[#f5f5f5] dark:bg-[#143029] dark:ring-1 dark:ring-[#3b7364]/55' : ''} content-stretch flex items-center justify-center p-[8px] relative rounded-[8px] shrink-0 cursor-pointer hover:bg-[#f5f5f5] dark:hover:bg-[#19362e] transition-colors`;

  return (
    <div className="bg-[#e0e1e9] dark:bg-[#0a1915] w-full relative" data-name="Header">
      ...
      <div
        aria-hidden="true"
        className="absolute bottom-0 left-0 right-0 border-b border-[#d9d9d9] dark:border-[#264d42] border-solid pointer-events-none"
      />
    </div>
  );
}
